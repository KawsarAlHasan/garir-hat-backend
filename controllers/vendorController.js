...
exports.signUpUser = async (req, res) => {
  try {
    ...
    const query =
      "INSERT INTO vendors (first_name, last_name, email, password, phone) VALUES (?, ?, ?, ?, ?)";
...
exports.updateUser = async (req, res) => {
  try {
    ...
    const [data] = await db.query(
      `UPDATE vendors SET first_name=?, last_name=?, phone=?, street_address=?, birth_day=?, city=?, postal_or_zip_code=?, state_or_region=?, country=? WHERE id = ?`,
      [
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    await db.query(`DELETE FROM vendors WHERE id=?`, [userID]);
    res.status(200).send({
      success: true,
